...
from src.gui_frame_ext.about_window import AboutWindow

from src.helpers.helper_validation import digit_validation
from src.dataset_management.dataset_manager import DatasetConfig
...
class GUIFrameExtractorConsole(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.input_folder_label = tk.Label(self.extract_folder_data_frame, text='Selected video folder:')
        self.input_folder_label.grid(row=0, column=0, sticky=tk.EW)

        self.input_folder_entry = tk.Entry(self.extract_folder_data_frame)
        self.input_folder_entry.grid(row=0, column=1, sticky=tk.EW)

        self.select_folder_button = tk.Button(self.extract_folder_data_frame, text='Select folder',command=self.select_folder_data)
        self.select_folder_button.grid(row=5, column=0, sticky=tk.EW)

        self.load_folder_button = tk.Button(self.extract_folder_data_frame, text='Extract frames from folder',command=self.extract_folder_data)
        self.load_folder_button.grid(row=5, column=1, sticky=tk.EW)

        ########################################################
        # extract from one file
        self.input_file_label = tk.Label(self.extract_data_frame, text='Selected video file:')
        self.input_file_label.grid(row=0, column=0, sticky=tk.EW)
        # # -----------------------
        self.input_file_entry = tk.Entry(self.extract_data_frame)
        self.input_file_entry.grid(row=0, column=1, sticky=tk.EW)

        self.offset_label = tk.Label(self.extract_data_frame, text='Offset to start in seconds:')
        self.offset_label.grid(row=1, column=0, sticky=tk.EW)
        # # -----------------------
        self.offset_entry = tk.Entry(self.extract_data_frame, validate="key")
        self.offset_entry.grid(row=1, column=1, sticky=tk.EW)
        self.offset_entry['validatecommand'] = (self.offset_entry.register(digit_validation), '%P', '%d')
        # # --------------
        self.number_of_frames_label = tk.Label(self.extract_data_frame, text='Number of frames:')
        self.number_of_frames_label.grid(row=2, column=0, sticky=tk.EW)
        # # -----------------------
        self.number_of_frames_entry = tk.Entry(self.extract_data_frame, validate="key")
        self.number_of_frames_entry.grid(row=2, column=1, sticky=tk.EW)
        self.number_of_frames_entry['validatecommand'] = (
            self.number_of_frames_entry.register(digit_validation), '%P', '%d')
        # # --------------
        # # insert default values
        self.offset_entry.insert(1, "1")  # todo: this may be loaded from a configuration file
        self.number_of_frames_entry.insert(1, "1")  # todo: this may be loaded from a configuration file
        ...
        self.load_file_button = tk.Button(self.extract_data_frame, text='Extract frames from file',
...
#        # remove_files(self.r_config.output_normalized_folder, self.r_config.file_type)
    ...
    def extract_folder_data(self):
        ...
        if directory_selected == "":
            ...
            ##################################
            an_offset = int(self.offset_entry.get())
            a_number_of_frames = int(self.number_of_frames_entry.get())
            ...
            self.frames_extractor_config.path_annotations_output = self.dataset_config.dataset_annotations_path  # TODO: 15/02/2022 temporal
        ...
        analyze_status_str = directory_selected
        self.messages_info.insert("1.0", analyze_status_str)
        self.results_info.insert("1.0", "Extracting from " + results_info_str)
    ...
    def extract_file_data(self):
        ...
        if path_filename_selected == "":
            ...
            print('a_number_of_frames -->', a_number_of_frames)
            self.frames_extractor_config.path_images_output = self.dataset_config.dataset_images_path
            self.frames_extractor_config.path_annotations_output = self.dataset_config.dataset_annotations_path  # 15/02/2022 temporal
            track_file = os.path.join(self.dataset_config.dataset_sets_path, 'all.txt')
            #########################
