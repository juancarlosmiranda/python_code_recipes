...
from src.mass_estimation import ComparativeMeasuresReport

class TestDatasetComparativeDayMeasures(unittest.TestCase):
    ...
    def test_dataset_comparative_day_measures(self):
        ...
        path_to_save_df = os.path.join(self.root_folder, 'output_dataset')
        path_day_measures = os.path.join(path_to_save_df, day_measures_filename)

        # create pandas dataframe with columns
        headers_day_comparative = [
            'date_capture', 'time_capture', 'fruit_id',
            'lab.tree', 'lab.fruit_label', 'lab.caliber_mm', 'lab.height_mm', 'lab.weight_gr', 'lab.observations',
            'pred.obj_detection', 'pred.measure_c_px', 'pred.measure_h_px', 'pred.depth', 'pred.caliber_mm',
            'pred.height_mm', 'pred.mass_gr'
        ]

        day_comparative_df = pd.DataFrame([], columns=headers_day_comparative)
        # ---------------------
        # here we open a list from a dataset, read from disc data saved
        ...
        # --------------
        # todo: here load input manual measures
        # todo: add selected fruits id
        ...
        measures_df = manual_measures_df[manual_measures_df["lab.tree"] == '2']  # todo: add parameter configuration
        ...
        for a_register in result_pair_list:
            ...
            # todo: merge by frame calculated data here
            temporal_record_merged = pd.merge(measures_df, temporal_record, left_on='fruit_id', right_on='fruit_id',
                                      how='inner')
            # ---------------------------
        ...
        day_comparative_df.to_csv(path_day_measures, float_format='%.2f', sep=';')
        # COMPARATIVE_DF ES USADO PARA LAS METRICAS POR FRAME
        # hay un resultado intermedio que se llama result_by_frame

        a_report = ComparativeMeasuresReport(day_comparative_df)
        a_report.print_weight_metrics()
        # todo: add here data to return
...
# todo: corroborar que las filas de lo manual son giaules a la manzana medida
# todo: agregar un test para el calculo de las metricas.